        ...
        {screen === "session" && current && (
          <div style={wrap}>
            ...
            >
              ...
              <div style={{ fontSize: 12, color: C.brass, letterSpacing: 0.6, marginTop: 8 }}>{sectionLabel(current)}</div>
              <h1 style={{ ...title, marginTop: 4, fontSize: "1.35rem" }}>{humanTitle(current.type)}</h1>
              <p style={{ color: C.textMuted, fontSize: 14, marginTop: 6, marginBottom: 0 }}>{current.instruction}</p>
            </div>
            {error && <p style={{ color: C.danger, marginTop: 12 }}>{error}</p>}
