...
export type SongPreviewLayer = {
  ...
  audio_url: string;
  /** Optional bar window (metadata; ms remains placement source of truth). */
...
type Props = {
  ...
  seed?: string;
  /**
   * Authoritative section membership. When set, layers with matching section_id
   * are included regardless of placement offset / start_ms window.
   */
  sectionFilterSectionId?: string | null;
  /** Fallback when layers lack section_id: filter by placement window. */
...
export function SongPreviewPlayer({
  ...
  const PREVIEW_BEAT_VOLUME = 0.0035;
  const layers = useMemo(() => {
    const hasSectionId = Boolean(sectionFilterSectionId);
    const hasMs =
      sectionFilterStartMs != null || sectionFilterEndMs != null;
    if (!hasSectionId && !hasMs) return layersIn;

    // Prefer explicit section_id membership (placement offset must not exclude a layer).
    if (hasSectionId) {
      const byId = layersIn.filter(
        (l) => l.section_id && l.section_id === sectionFilterSectionId
      );
      if (byId.length > 0) return byId;
      // Fall through to ms window only if no layer carries section_id yet (legacy).
      const anySectionIds = layersIn.some((l) => Boolean(l.section_id));
      if (anySectionIds) {
        // Layers have section_ids but none match — do not leak other sections via ms.
        return byId;
      }
    }

    if (!hasMs) return layersIn;
  ...
  }, [layersIn, sectionFilterSectionId, sectionFilterStartMs, sectionFilterEndMs]);
      ...
      const tick = () => {
        ...
        if (beat && !beat.paused) {
          try {
            ...
            if (Math.abs(beat.volume - PREVIEW_BEAT_VOLUME) > 0.002) {
              beat.volume = PREVIEW_BEAT_VOLUME;
            }
          } catch {
            /* ignore */
