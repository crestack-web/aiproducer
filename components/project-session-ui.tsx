...
export default function ProjectDetailPage() {
  ...
  function beginMediaCapture(stream: MediaStream, task: Task) {
    ...
    if (beatAudioRef.current && beatUrl) {
      ...
      const isSpeaker = (selectedSpeakerIdRef.current || "").includes("speaker") || selectedSpeakerIdRef.current === "__speaker__";
      beatAudioRef.current.muted = false;
      // Phone speaker: keep monitor low to reduce acoustic bleed into phone mic
      beatAudioRef.current.volume = isSpeaker ? 0.05 : 0.1;
            ...
            {(planTasks.length > 0 || tasks.length > 0) && (
              <>
                ...
                <button
                  ...
                  disabled={analyzing || tasks.filter((t) => t.status !== "skipped").length === 0}
                >
                  Start recording
                </button>
