...
function IconLibrary({ size = 20, color = "currentColor" }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden>
      <path d="M5 4.5h10.5A2.5 2.5 0 0 1 18 7v12.2a.8.8 0 0 1-1.2.7L12 17.2l-4.8 2.7A.8.8 0 0 1 6 19.2V7A2.5 2.5 0 0 0 3.5 4.5" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M9 8.5h5" stroke={color} strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}
...
function IconMusic({ size = 28, color = "rgba(244,241,236,0.88)" }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden>
      <path d="M9 18.5a2.75 2.75 0 1 1-2.1-2.68V7.4c0-.7.46-1.32 1.14-1.52L18 3.2v11.9" stroke={color} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
          ...
          return (
            <Link key={p.id} href={`/app/projects/${p.id}`} style={S.projectCard}>
              <div style={{ ...S.cover, background: `linear-gradient(145deg, ${g[0]}, ${g[1]})` }}>
                <div style={S.coverIcon}><IconMusic size={32} /></div>
              </div>
    ...
    <div style={S.shell}>
      ...
      <aside className="studio-sidebar" style={S.sidebar}>
        ...
        <nav style={S.nav}>
          {(["home", "library", "profile"] as Tab[]).map((key) => (
            <button key={key} type="button" onClick={() => setTab(key)} style={{ ...S.navItem, ...(tab === key ? S.navActive : {}) }}>
              <span style={S.navIcon}>
                {key === "home" && <IconHome size={18} color={tab === key ? C.brass : C.textMuted} />}
                {key === "library" && <IconLibrary size={18} color={tab === key ? C.brass : C.textMuted} />}
                {key === "profile" && <IconProfile size={18} color={tab === key ? C.brass : C.textMuted} />}
              </span>
            ...
            <>
              <div className="studio-hero-row" style={S.heroRow}>
                ...
                <div className="studio-hero-art" style={S.heroArt}>
                  ...
                  <div style={S.coverIcon}><IconMusic size={42} /></div>
                </div>
              </div>
      ...
      <nav className="studio-bottom-nav" style={S.bottomNav} aria-label="Main">
        {(["home", "library", "profile"] as Tab[]).map((key) => (
          ...
          >
            {key === "home" && <IconHome size={22} color={tab === key ? C.brass : C.textFaint} />}
            {key === "library" && <IconLibrary size={22} color={tab === key ? C.brass : C.textFaint} />}
            {key === "profile" && <IconProfile size={22} color={tab === key ? C.brass : C.textFaint} />}
            {key === "home" ? "Home" : key === "library" ? "Library" : "Profile"}
          </button>
        ))}
      </nav>
...
const S: Record<string, React.CSSProperties> = {
  ...
  navIcon: { display: "inline-flex", width: 20, height: 20, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  coverIcon: { position: "absolute", inset: 0, display: "grid", placeItems: "center", pointerEvents: "none" },
  ...
  cover: { width: "100%", aspectRatio: "1", borderRadius: 14, border: `1px solid ${C.border}`, position: "relative", overflow: "hidden" },
  ...
  bottomItem: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4, color: C.textFaint, fontSize: 10.5, fontWeight: 500, fontFamily: "inherit", background: "transparent", border: "none", cursor: "pointer", paddingBottom: 8 },
